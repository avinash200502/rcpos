...
const initialEmployees = [
  ...
  // Add 6 more employee objects here as needed for example
];
...
export default function EmployeeManagement() {
  ...
  const emptyEmployeeForm = {
    ...
    emergencyContacts: [
      { name: "", phoneCountryCode: "+27", phone: "" },
    ],
    status: true,
  };
  ...
  const validateForm = () => {
    const newErrors = {};
    if (!validateRequired(form.firstName)) newErrors.firstName = "First name is required";
    if (!validateRequired(form.lastName)) newErrors.lastName = "Last name is required";
    if (!validateRequired(form.phone)) newErrors.phone = "Phone number is required";
    ...
    if (!validateRequired(form.currentAddress.country)) newErrors["currentAddress.country"] = "Country is required";
    ...
    if (!validateRequired(form.permanentAddress.country)) newErrors["permanentAddress.country"] = "Country is required";
  ...
  const onFormSubmit = (e) => {
    ...
    if (isAddingNew) {
      ...
      setEmployees(prev =>
        prev.map(emp => emp.id === editingEmployeeId ? { ...form, id: editingEmployeeId, profileImage: null } : emp)
      );
...
<div className="max-w-full mx-auto px-10 bg-white rounded-lg shadow-md font-sans">
        ...
        <h2 className="text-2xl font-semibold mb-3">{isAddingNew ? "Add Employee Profile" : "Edit Employee Profile"}</h2>
        ...
        <form onSubmit={onFormSubmit} noValidate>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <input type="text" value={form.empId} readOnly className="w-full p-2 border rounded bg-gray-100 cursor-not-allowed" />
              <label className="block font-medium mt-4 mb-1">Salutation</label>
              <select
                name="salutation"
          ...
          {/* Personal Details */}
          <h3 className="text-xl font-semibold mt-8 mb-4">Personal Details</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <input
                ...
                onChange={onInputChange}
                className={`w-full p-2 border rounded ${errors.firstName ? "border-red-600" : "border-gray-300"}`}
              />
              {errors.firstName && <p className="text-red-600 text-sm mt-1">{errors.firstName}</p>}

              <label className="block font-medium mt-4 mb-1">Company</label>
              <select
                name="company"
                value={form.company}
                onChange={onInputChange}
              ...
              <label className="block font-medium mt-4 mb-1">Designation</label>
              <select
                name="designation"
              ...
              <div className="flex space-x-2">
                ...
                <input
                  ...
                  className={`flex-grow p-2 border rounded ${errors.phone ? "border-red-600" : "border-gray-300"}`}
                />
              </div>
              {errors.phone && <p className="text-red-600 text-sm mt-1">{errors.phone}</p>}

              <label className="block font-medium mt-4 mb-1">Date of Birth</label>
              <input
                type="date"
                name="dob"
                value={form.dob}
            ...
            <div>
              ...
              <input
                ...
                value={form.lastName}
                onChange={onInputChange}
                className={`w-full p-2 border rounded ${errors.lastName ? "border-red-600" : "border-gray-300"}`}
              />
              {errors.lastName && <p className="text-red-600 text-sm mt-1">{errors.lastName}</p>}

              <label className="block font-medium mt-4 mb-1">Department</label>
              <select
                name="department"
              ...
              <input
                ...
                className={`w-full p-2 border rounded ${errors.email ? "border-red-600" : "border-gray-300"}`}
              />
              {errors.email && <p className="text-red-600 text-sm mt-1">{errors.email}</p>}

              <label className="block font-medium mt-4 mb-1">Gender</label>
              <select
                name="gender"
                value={form.gender}
                onChange={onInputChange}
          ...
          {/* Address Section */}
          <h3 className="text-xl font-semibold mt-8 mb-4">Current Address</h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
              <label className="block font-medium mb-1">House/Flat No.</label>
              <input
                ...
                className="w-full p-2 border rounded"
              />
              <label className="block font-medium mt-4 mb-1">Area/Street/Village</label>
              <input
            ...
            <div>
              ...
              <input
                ...
                className="w-full p-2 border rounded"
              />
              <label className="block font-medium mt-4 mb-1">Pincode</label>
              <input
                name="currentAddress.pincode"
                value={form.currentAddress.pincode}
                onChange={onInputChange}
                maxLength={6}
                className={`w-full p-2 border rounded ${errors["currentAddress.pincode"] ? "border-red-600" : "border-gray-300"}`}
              />
              {errors["currentAddress.pincode"] && <p className="text-red-600 text-sm mt-1">{errors["currentAddress.pincode"]}</p>}
            </div>
            <div>
              ...
              >
                ...
                {(provincesByCountry[form.currentAddress.country] || []).map(p => (
              ...
              <select
                ...
                onChange={onInputChange}
                className={`w-full p-2 border rounded ${errors["currentAddress.country"] ? "border-red-600" : "border-gray-300"}`}
              >
              ...
              {errors["currentAddress.country"] && <p className="text-red-600 text-sm mt-1">{errors["currentAddress.country"]}</p>}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div>
              ...
              <input
                ...
                onChange={onInputChange}
                maxLength={6}
                className={`w-full p-2 border rounded ${errors["permanentAddress.pincode"] ? "border-red-600" : "border-gray-300"}`}
              />
              {errors["permanentAddress.pincode"] && <p className="text-red-600 text-sm mt-1">{errors["permanentAddress.pincode"]}</p>}
            </div>
            <div>
              <label className="block font-medium mb-1">Province</label>
              <select
              ...
              >
                <option value="">Select Province</option>
                {(provincesByCountry[form.permanentAddress.country] || []).map(p => (
                  <option key={p} value={p}>{p}</option>
                ))}
              </select>
              <label className="block font-medium mt-4 mb-1">
              ...
              <select
                ...
                onChange={onInputChange}
                className={`w-full p-2 border rounded ${errors["permanentAddress.country"] ? "border-red-600" : "border-gray-300"}`}
              >
                <option value="">Select Country</option>
                {Object.keys(provincesByCountry).map(c => (
                  <option key={c} value={c}>{c}</option>
              ...
              {errors["permanentAddress.country"] && <p className="text-red-600 text-sm mt-1">{errors["permanentAddress.country"]}</p>}
            </div>
          </div>

          {/* Emergency Contacts */}
          <h3 className="text-xl font-semibold mt-8 mb-4">Emergency Contact(s)</h3>
            ...
            <div key={idx} className="flex gap-4 mb-4 items-end">
              <div className="flex-1">
                ...
                <input
                  ...
                  className={`w-full p-2 border rounded ${errors[`emergencyContacts.${idx}.name`] ? "border-red-600" : "border-gray-300"}`}
                  placeholder="Name"
                />
                {errors[`emergencyContacts.${idx}.name`] && (
                  <p className="text-red-600 text-sm mt-1">{errors[`emergencyContacts.${idx}.name`]}</p>
                )}
              </div>
              <div className="flex-1">
                ...
                <div className="flex gap-2">
                  <select
                    ...
                    onChange={e => onEmergencyContactChange(idx, "phoneCountryCode", e.target.value)}
                    className="p-2 border rounded w-24"
                  >
                    {countryCodes.map(cc => <option key={cc} value={cc}>{cc}</option>)}
                  </select>
                  <input
                    value={c.phone}
                    onChange={e => onEmergencyContactChange(idx, "phone", e.target.value)}
                    maxLength={15}
                    className={`flex-grow p-2 border rounded ${errors[`emergencyContacts.${idx}.phone`] ? "border-red-600" : "border-gray-300"}`}
                    placeholder="Phone number"
                  />
                </div>
                {errors[`emergencyContacts.${idx}.phone`] && (
                  <p className="text-red-600 text-sm mt-1">{errors[`emergencyContacts.${idx}.phone`]}</p>
                )}
              </div>
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          <nav className="text-gray-600 text-sm mb-1">
            <span>Dashboards</span> / <span className="font-semibold text-gray-900">Employee List</span>
          </nav>
 ...
 Add Employee
      ...
      <div className="overflow-x-auto rounded border border-gray-200">
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              {["S.NO", "EMPLOYEE ID", "FIRST NAME", "DEPARTMENT", "PHONE", "EMAIL", "COMPANY", "STATUS", "ACTIONS"].map((heading) => (
                <th
                  key={heading}
          ...
          <tbody className="divide-y divide-gray-200">
            {paginatedEmployees.length > 0 ? paginatedEmployees.map((emp, ix) => (
              <tr key={emp.id} className="hover:bg-gray-50">
                <td className="px-4 py-3 whitespace-nowrap">{(currentPage - 1) * entriesPerPage + ix + 1}</td>
                <td className="px-4 py-3 whitespace-nowrap">{emp.empId}</td>
                <td className="px-4 py-3 whitespace-nowrap">{emp.salutation} {emp.firstName} {emp.lastName}</td>
                <td className="px-4 py-3 whitespace-nowrap">{emp.department}</td>
                <td className="px-4 py-3 whitespace-nowrap">{emp.phoneCountryCode} {emp.phone}</td>
                <td className="px-4 py-3 whitespace-nowrap">{emp.email}</td>
                <td className="px-4 py-3 whitespace-nowrap">{emp.company}</td>
               ...
               <label className="inline-flex relative items-center cursor-pointer">
                          <input
                            ...
                            aria-label={`Toggle status for ${emp.designation}`}
                          />
  ...
  >
    <Trash2 size={18} />
              ...
              <tr>
                <td colSpan="9" className="text-center py-6 text-gray-800 select-none">No employees found.</td>
              </tr>
            )}
          </tbody>
        </table>
      ...
      <div className="flex justify-center space-x-2 my-6">
        ...
        <button
          ...
          className={`px-3 py-1 rounded ${currentPage === totalPages || totalPages === 0 ? "bg-gray-200 cursor-not-allowed" : "bg-gray-100 hover:bg-gray-300"}`}
        >
          Next
        </button>
      </div>
